...
use GXModules\VRPaymentPayment\Library\Core\{
...
class VRPaymentWebhookController extends HttpViewController
{
	/**
	...
	public function actionIndex()
	{
		...
		switch ($listenerEntityTechnicalName) {
			...
			case WebHookRequest::REFUND:
				...
				VRPaymentRefundModel::createRefundRecord((int)$data['entityId'], $orderId, $refund->getAmount());
				$refunds = VRPaymentRefundModel::getRefunds($orderId);
				$amountToBeRefunded = floatval($transactionInfo['info']['total']) - VRPaymentRefundModel::getTotalRefundsAmount($refunds);
