...
use GXModules\VRPaymentPayment\Library\{Core\Settings\Options\Integration,
    Core\Settings\Struct\Settings,
    Helper\VRPaymentHelper
};
use VRPayment\Sdk\Model\{AddressCreate, LineItemCreate, LineItemType, Transaction};
use VRPayment\Sdk\Model\TransactionPending;

class VRPayment_CheckoutProcessProcess extends VRPayment_CheckoutProcessProcess_parent
{
    /**
    ...
    {
        if (strpos($_SESSION['payment'] ?? '', 'vrpayment') === false) {
            parent::proceed();
            return true;
        }
        ...
        if ($this->tmp_order === false) {
            ...
            $this->set_redirect_url(xtc_href_link("shop.php", 'do=VRPaymentPayment/PaymentPage', 'SSL'));
            return true;
        }
    ...
    private function confirmTransaction(Transaction $transaction, string $orderId, Settings $settings): Transaction
    {
        ...
        if ($giftCouponLineItem) {
		  ...
		  // NEU 08.05.2026 Ermittlung des Rabatts auf der Bestellung / dm
    ...
    {
        $vrpaymentPaymentType = $_SESSION['payment_methods_title'] ?? null;
        if (empty($vrpaymentPaymentType) || strpos($_SESSION['payment'], 'vrpayment') === false) {
    ...
    /**
     * NEU 08.05.2026 Discount auf der Bestellung / dm
     * @return LineItemCreate|null
     */
