...
use JTL\Shop;
use VRPayment\Sdk\Model\{AddressCreate, LineItemCreate, LineItemType, Transaction, TransactionPending};

class VRPaymentCheckoutPaymentModulesThemeContentView extends VRPaymentCheckoutPaymentModulesThemeContentView_parent
{
    ...
    public function __construct()
    {
        ...
        $this->set_content_template('GXModules/VRPayment/VRPaymentPayment/Shop/Themes/All/checkout_payment_modules.html');
        $this->set_flat_assigns(true);
    }

    public function prepare_data()
    {
        $key = 'configuration/MODULE_PAYMENT_VRPAYMENT_STATUS';
            ...
            if ($order->info['total'] > 0) {
                ...
                if (!$arrayOfPossibleMethods) {
                    ...
                    foreach ($possiblePaymentMethods as $possiblePaymentMethod) {
                        $arrayOfPossibleMethods[] = 'vrpayment_' . VRPaymentHelper::slugify($possiblePaymentMethod->getName(), '_');
                    }
                ...
                $paymentMethodConfKey = 'configuration/MODULE_PAYMENT_VRPAYMENT_';
                ...
                $array = [];
                foreach ($selection as $t_key => $t_method_array) {
                    if (strpos(strtolower($t_method_array['id']), 'vrpayment') !== false) {
                        if (strtolower($t_method_array['id']) !== 'vrpayment') {
